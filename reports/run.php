 ...
 */
	...
	$continuar = (( isset($_SESSION['iglesia_id']) or isset($_SESSION['run_ok']) ) ? 1 : 0);
	if( $continuar ==0  ) {
		header('HTTP/1.1 403 Forbidden');
		print '<h1>Error: Acceso restringido<br/></h1>';
		die("Usted no cuenta con acceso a esta secci&oacute;n.");
	}
	//-----------------------------------------------------------------------------------
    ...
    */
    ...
    //define('SW_FRAMEWORK_DB_USER', $db['default']['username']);//'<USER>'
    //define('SW_FRAMEWORK_DB_PASSWORD', $db['default']['password']);//'PASSWORD'
    //define('SW_FRAMEWORK_DB_HOST', $db['default']['hostname']);//'127.0.0.1' // Use ip:port to specifiy a non standard port
    //define('SW_FRAMEWORK_DB_DATABASE', $db['default']['database']);//'<DATABASENAME>'
    //
